...
export const Card: React.FC<CardProps> = ({ job, onSwipe, isActive = true, forcedSwipe }) => {
  const [offset, setOffset] = React.useState(0);
  const [rotation, setRotation] = React.useState(0);
  const [isLeaving, setIsLeaving] = React.useState(false);

  // Generate a consistent gradient for each job based on its ID
  const gradientIndex = parseInt(job.id.replace(/\D/g, ''), 10) % gradients.length;
  ...
  return (
    <div
      {...(isActive ? handlers : {})}
      className={`w-full h-full select-none ${isActive ? 'cursor-grab active:cursor-grabbing' : ''}`}
      style={{
        transform: `translateX(${offset}px) rotate(${rotation}deg)`,
        transition: isLeaving || (offset === 0) ? 'transform 0.3s ease-out' : 'none',
    ...
    >
      <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-xl bg-white dark:bg-gray-800 transition-colors">
        <div className={`h-40 sm:h-36 ${gradient} flex items-center justify-center relative overflow-hidden`}>
        ...
        <div className="p-6">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white transition-colors">{job.title}</h2>
